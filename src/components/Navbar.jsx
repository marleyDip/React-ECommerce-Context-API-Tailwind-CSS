...
const Navbar = ({ location, getLocation, openDropDown, setOpenDropDown }) => {
  ...
  return (
    //<div className="fixed left-0 w-full top-0 bg-white py-3 shadow-2xl px-4 md:px-0 z-50">
    <div className="sticky top-0 bg-white py-3 shadow-2xl px-4 md:px-0 z-50">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-5 lg:gap-7">
          ...
          {/* location */}
          <div className="group md:flex hidden items-center gap-0 lg:gap-1 text-gray-700">
            <MapPin className="text-red-500 transform group-hover:scale-110 lg:group-hover:scale-125 active:95 transition-transform duration-200 ease-out" />
          ...
          {openDropDown ? (
            <div className="w-[180px] lg:w-[220px] h-max shadow-2xl z-50 bg-white fixed top-16 left-60 p-4 border-2 border-gray-100 rounded-md">
              <h1 className="text-base font-medium lg:font-semibold lg:text-lg relative">
                Change Location
                <span>
                  <CgClose
                    onClick={toggleDropDown}
              ...
              <button
                onClick={getLocation}
                className="text-sm lg:text-base bg-red-500 text-white mt-2 px-3 py-1.5 rounded-md shadow-md hover:shadow-lg hover:rounded-lg hover:bg-red-400 transform hover:translate-x-1 hover:-translate-y-0.5 duration-200 ease-in-out cursor-pointer"
              >
                Detect my location
              </button>
            </div>
        ...
        <nav className="flex items-center gap-5 lg:gap-7">
          {/* desktop menu */}
          <ul className="md:flex hidden  items-center gap-4 lg:gap-7 text-xl font-semibold">
            <NavLink
              to={"/"}
              className={({ isActive }) =>
                `${
